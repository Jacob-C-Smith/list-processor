...
int main ( int argc, const char *argv[] )
{
    ...
    while ( !feof(stdin) )
    {
        ...
        // Error check
        if ( strlen(p_token) == NULL ) return EXIT_FAILURE;
        ...
        while ( *p_token != '\0' )
        {
            ...
            if ( *p_token ==  '(' ) { for (size_t i = 0; i < t; i++) printf("  "); putchar('('); t++, p_token++; continue; }
            if ( *p_token ==  ')' ) { putchar(')'); t--, p_token++; for (size_t i = 0; i < t; i++) printf("  "); continue; }
            
            for (size_t i = 0; i < t; i++) printf("  ");
            
            do  { putchar(*p_token), p_token++; } 
